...
/*
 * NSSound's libao sink ignores -setVolume:, and lowering the mixer for the
 * alert would also duck every other sound. Alerts therefore play through
 ...
 * -initWithData:. Only the alert itself plays quieter; nothing on the
...
- (void)beep
{
    ...
    @autoreleasepool {
        ...
        if (prefs) {
            ...
            /* The Sound prefPane's alert volume slider is the linear gain
             * for alerts; without it alerts play at full level */
            float gain = alertVolume ? [alertVolume floatValue] : 1.0;
                ...
                for (NSString *basePath in soundPaths) {
                    for (NSString *ext in extensions) {
                        ...
                        if ([[NSFileManager defaultManager] fileExistsAtPath:soundPath]) {
                            ...
                            // Prefer the attenuated copy so only the alert,
